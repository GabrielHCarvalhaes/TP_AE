...
public class Main{
    ...
    public static void insercao(int[] ids, int tam){
        for(int i=1; i<tam; i++){
            String tmp = show[ids[i]-1].getType();
            String tmp1 = show[ids[i]-1].getTitle();
            int temp = ids[i];
            int j = i-1;
            boolean titlemenor = true;
            while (j>=0 && show[ids[j]-1].getType().toLowerCase().compareTo(tmp.toLowerCase()) >= 0 && titlemenor) {
                if(show[ids[j]-1].getType().toLowerCase().compareTo(tmp.toLowerCase()) == 0){
                    if(show[ids[j]-1].getTitle().toLowerCase().compareTo(tmp1.toLowerCase()) > 0){
                        ids[j+1] = ids[j];
                        j--;
