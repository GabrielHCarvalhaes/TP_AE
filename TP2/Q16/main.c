...
int compararShows(int id1, int id2) {
    ...
    // Comparação case-insensitive para type
    int cmpType;
    #ifdef _WIN32
        cmpType = _stricmp(show1->type, show2->type);
    #else
        cmpType = strcasecmp(show1->type, show2->type);
    #endif
    
    if (cmpType != 0) return cmpType;
    
    // Comparação case-insensitive para title se types forem iguais
    #ifdef _WIN32
...
// Versão otimizada da inserção parcial com k iterações
void insercaoParcial(int ids[], int tam, int k) {
    // Executa k iterações do insertion sort
    for (int iter = 0; iter < k; iter++) {
        // A cada iteração, ordena mais um elemento
        for (int i = 1; i < tam; i++) {
            int current = ids[i];
            int j = i - 1;
            
            // Move elementos maiores para frente
            while (j >= 0 && compararShows(current, ids[j]) < 0) {
                ids[j + 1] = ids[j];
                j--;
            }
            ids[j + 1] = current;
            
            // Interrompe após k elementos na primeira iteração
            if (iter == 0 && i >= k) break;
        }
    }
}
...
int main() {
    ...
    // Chama a inserção parcial com 10 iterações
    insercaoParcial(ids, tam, 10);
