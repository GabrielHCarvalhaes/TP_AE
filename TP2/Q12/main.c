...
// Função de comparação principal 
...
// Função BubbleSort recursiva
void bubbleSortRecursivo(int ids[], int tam, int passada) {
    if (passada >= tam - 1) return; // Caso base: todas as passadas completas
    
    // Variável para controlar se houve trocas nesta passada
    int trocou = 0;
    
    // Faz uma passada do BubbleSort
    for (int i = 0; i < tam - passada - 1; i++) {
        if (compararShows(ids[i], ids[i+1]) > 0) {
    ...
    // Chamada recursiva para a próxima passada
    bubbleSortRecursivo(ids, tam, passada + 1);
}

// Função de interface para o BubbleSort
void bubbleSort(int ids[], int tam) {
    bubbleSortRecursivo(ids, tam, 0);
}
