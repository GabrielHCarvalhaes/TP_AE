...
int compararShows(int id1, int id2) {
    ...
    // Comparação principal por type (case insensitive)
    int cmpType = strcasecmp(show1->type, show2->type);
    if (cmpType != 0) {
        return cmpType;
    }
...
void shellSort(int ids[], int tam) {
    // Sequência de gaps (pode ser ajustada)
    for (int gap = tam/2; gap > 0; gap /= 2) {
        // Fazer insertion sort para este gap
        for (int i = gap; i < tam; i++) {
            int temp = ids[i];
            int j;
            
            // Deslocar elementos até encontrar a posição correta
            for (j = i; j >= gap && compararShows(ids[j - gap], temp) > 0; j -= gap) {
                ids[j] = ids[j - gap];
            }
